...
# fillingMode.sh — Description
# Usage: fillingMode.sh
...
awk -F'\t' -v month=$1 -v year=$YEAR '
	...
	$2 ~ sprintf("%02d",month)"/"year {
		# this check allows us to read the first OFFLINE reading of each load and then no other lines, therefore if the same load number is filled twice only the first loading data is used
		if(offCount[$1]!=0) { next }
		# increment offCount on detection that we have finished pumping
		if($8=="OFFLINE") { offCount[$1]++ }
		if($8=="LOADING" && ++loadCount==1)	{ first=$1 } else { if($1>last) last=$1 }	# find the first & last load for the for loop
