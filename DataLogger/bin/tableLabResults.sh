...
tac "${LOGS_DIR}/lab_log.txt" | awk -v filter="$1" -v lines="$2" '
	...
	$0 ~ filter {
		...
		print "<a href=\"file:///Certification/Testing/" sampleDate[3] "/" sprintf("%08d",$1) ".pdf\">"$1"</a>" OFS $16 OFS $2 OFS $3 OFS $4 OFS2 $5 OFS2 $6 OFS2 $7 OFS2 $8 OFS $9 OFS $10 OFS $11 OFS $12 OFS $13 OFS $14 OFS "&pound;" $15;
		print "</td></tr>";
		if(NR==lines) { exit }
	}
#	lines==1 && NR>75	{ exit }
