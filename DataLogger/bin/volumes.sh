...
awk -v yr="$YEAR" -v curYr="$CURYEAR" -v finyr="$FINYR" -v allYrs="$ALLYEARS" -v volFile="$LOGS_DIR/vol.dat" -v capFile="$LOGS_DIR/capital.dat" '
	BEGIN {	FS=OFS="\t";
...
#			vol = vol "\n";					# print newline
#		}
#		printf vol > "$LOGS_DIR/vol.dat";
######
###### OR
######
		print "#Month" volHeader > volFile;				# print header
		for(i=1; i<=12; i++) {
			printf "%s\t", m[i] > volFile;			# print month name
			for(y=2016; y<=curYr; y++)
				printf "%d\t", loads[y][i] > volFile;		# print no. of loads per month for each year
			printf "\n" > volFile;				# print newline
		}
...
#		for(y=2016; y<=curYr; y++)
		...
		# printf "Oustanding %.2f\n", totalSpent  > capFile; # commented out line as we now display the number of remaining loads to clear the capital
#		printf "Oustanding (%.1f Loads)\t%.2f\n", totalSpent / capLoad, totalSpent  > capFile;
		#### end of capital data ####
#####
