...
//		$output = shell_exec ("tableLabCosts.sh" );
		$output = shell_exec ("tableLabCostsYr.sh -y ".escapeshellarg(strftime('%Y')));
	}
// TESTING to show prevoius years
		...
		putenv('LC_ALL='.$locale);
			...
			if($_GET['year'][4]=="/"){
			        ...
			        $output = shell_exec("tableLabCostsYr.sh -fy ".escapeshellarg($finyear) );
			}
			else if($_GET['year']=='all')
			        $output = shell_exec("tableLabCostsYr.sh -a");
			else
			        $output = shell_exec("tableLabCostsYr.sh -y ".escapeshellarg( $_GET['year'] ));
	}
// END OF TESTING
